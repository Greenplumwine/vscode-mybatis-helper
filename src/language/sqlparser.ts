...
export class SQLParser {
  ...
  setDatabaseType(type: DatabaseType): void {
    try {
      ...
      logger.error(
        `Error setting database type: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
    ...
    try {
      ...
      if (preparingPattern) {
        try {
          ...
          logger.error(
            `Error setting preparing regex: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      ...
      if (parametersPattern) {
        try {
          ...
          logger.error(
            `Error setting parameters regex: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
      ...
      if (executionTimePattern) {
        try {
          ...
          logger.error(
            `Error setting execution time regex: ${error instanceof Error ? error.message : String(error)}`,
          );
        }
      }
    } catch (error) {
      logger.error(
        `Error setting custom patterns: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  resetToDefaultPatterns(): void {
    try {
      ...
      logger.error(
        `Error resetting to default patterns: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  parsePreparingLog(line: string): string | null {
    try {
      ...
      logger.error(
        `Error parsing preparing log: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  parseParametersLog(
    ...
    try {
      ...
      while (
        (paramMatch = this.paramRegex.exec(paramsString)) !== null &&
        paramCount < paramBatchSize
      ) {
      ...
      logger.error(
        `Error parsing parameters log: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  parseExecutionTimeLog(line: string): number | null {
    try {
      ...
      logger.error(
        `Error parsing execution time log: ${error instanceof Error ? error.message : String(error)}`,
      );
      return null;
    }
  }
  ...
  fillParametersToSQL(
    ...
    try {
      ...
      logger.error(
        `Error filling parameters to SQL: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private processArrayParameters(
    ...
    for (let i = 0; i < params.length; i++) {
      ...
      try {
        ...
        const arrayValue = param.value.replace(/\[/g, "").replace(/\]/g, "");
        ...
        resultSQL = resultSQL.replace(inClauseRegex, `IN (${formattedItems})`);
      } catch (error) {
        logger.warn(
  ...
  private getArrayItemType(arrayType: string): string {
    try {
      ...
      logger.error(
        `Error inferring array item type: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private formatBooleanParam(value: string): string {
    try {
      ...
      logger.error(
        `Error formatting boolean parameter: ${error instanceof Error ? error.message : String(error)}`,
      );
      return "NULL";
    }
  }
    ...
    try {
      ...
      const arrayFormats: Record<DatabaseType, string> = {
        [DatabaseType.POSTGRESQL]:
          formattedArray.startsWith("[") && formattedArray.endsWith("]")
      ...
      // Handle NULL values
        ...
        // Check if it's a JSON type
          ...
          try {
            ...
            logger.warn(
              `Invalid JSON value: ${trimmedValue} for type: ${type}`,
            );
            return "NULL";
          }
        } else {
  ...
  processSQLQuery(query: SQLQuery): SQLQuery {
    try {
      ...
      logger.error(
        `Error processing SQL query: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  private extractSQLOperationType(sql: string): string {
    try {
      ...
      logger.warn(
        `Error extracting SQL operation type: ${error instanceof Error ? error.message : String(error)}`,
      );
      return "SQL";
    }
  }
