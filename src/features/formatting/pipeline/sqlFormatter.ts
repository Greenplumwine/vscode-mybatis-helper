...
export class SqlFormatter implements PipelineStep {
  ...
  execute(content: string, context: PipelineContext): string {
    ...
    for (const region of context.sqlRegions) {
      try {
        // 格式化包含标签占位符的 SQL
        const formatted = this.formatWithPlaceholders(region.sqlContent, context.options);
  ...
  private formatWithPlaceholders(sql: string, options: PipelineContext['options']): string {
    // 保护标签占位符（__TAG_X_Y__）
    ...
    const formatted = formatSqlLib(tempSql, {
      language: options.sqlDialect as any,
