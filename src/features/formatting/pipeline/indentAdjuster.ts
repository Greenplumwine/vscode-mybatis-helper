...
export class IndentAdjuster implements PipelineStep {
  ...
  execute(content: string, context: PipelineContext): string {
    ...
    for (const region of context.sqlRegions) {
      // 计算 SQL 应有的基础缩进
      const baseIndent = this.calculateBaseIndent(content, region.placeholder, tabSize);
      
      // 调整 SQL 缩进
      region.sqlContent = this.adjustIndent(region.sqlContent, baseIndent, tabSize);
    }
    
    return content;
  }

  private calculateBaseIndent(content: string, placeholder: string, tabSize: number): number {
  ...
  private adjustIndent(sql: string, baseIndentLevel: number, tabSize: number): string {
