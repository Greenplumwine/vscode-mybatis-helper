...
export class UnifiedNavigationService {
  ...
  public async navigateJavaToXml(
    ...
    this.logger?.debug(
      `[Navigate] Java→XML: ${javaPath}, method: ${methodName}`,
    );
    ...
    try {
      ...
      this.logger?.debug(
        `[Navigate] Mapping found: ${mapping ? "yes" : "no"}, xmlPath: ${mapping?.xmlPath || "none"}`,
      );

      // 2. 如果没有映射，尝试动态解析 Java 文件
      if (!mapping) {
        this.logger?.info(
          `[Navigate] No mapping found for ${javaPath}, trying dynamic parse...`,
        );
      ...
      if (!mapping.xmlPath) {
        ...
        if (xmlPath) {
          ...
          this.logger?.warn(
            `[Navigate] XML not found for namespace: ${mapping.namespace}`,
          );
          vscode.window.showWarningMessage(
      ...
      this.logger?.error(
        "[Navigate] Java→XML navigation failed:",
        error as Error,
      );
  ...
  public async navigateXmlToJava(
    ...
    try {
      ...
      this.logger?.debug(
        `[Navigate] Mapping found by XML path: ${mapping ? "yes" : "no"}`,
      );
      ...
      try {
        ...
        this.logger?.warn(
          `[Navigate] Java file not found: ${mapping.javaPath}`,
        );
      ...
      if (sqlId) {
        ...
        if (!targetPosition) {
          this.logger?.debug(
            `[Navigate] Method position not in index, finding dynamically: ${sqlId}`,
          );
          targetPosition = await this.findMethodPositionDynamically(
            mapping.javaPath,
            sqlId,
          );
        }
      ...
      this.logger?.debug(
        `[Navigate] XML→Java completed in ${Date.now() - startTime}ms`,
      );
      return true;
    } catch (error) {
      this.logger?.error(
        "[Navigate] XML→Java navigation failed:",
        error as Error,
      );
      vscode.window.showErrorMessage(vscode.l10n.t("error.navigationFailed"));
      return false;
      ...
      for (const file of files) {
        try {
          ...
          if (
            ...
            actualPackage === namespace.substring(0, namespace.lastIndexOf("."))
          ) {
            // 找到匹配的 Java 文件，创建映射
  ...
  private extractMethodsFromContent(
    content: string,
  ): Array<{ name: string; position: { line: number; column: number } }> {
    const methods: Array<{
      name: string;
    ...
    const methodRegex =
      /^(?:\s*)(?:public|private|protected)?\s*(?:static|final|abstract)?\s*[\w<>,\s]+\s+(\w+)\s*\([^)]*\)\s*(?:throws\s+[\w,\s]+)?\s*[;{]/;
  ...
  private findSqlIdPositionInJava(
    ...
    if (
      methodMapping.javaPosition.line === 0 &&
      methodMapping.javaPosition.column === 0
    ) {
   ...
   */
  private async findMethodPositionDynamically(
    javaPath: string,
    methodName: string,
    ...
    try {
      ...
      >("vscode.executeDocumentSymbolProvider", vscode.Uri.file(javaPath));
      ...
      this.logger?.debug(
        `[findMethodPositionDynamically] Failed to get symbols:`,
        error,
      );
    }
  ...
  private async openAndReveal(
    ...
    const editor = await vscode.window.showTextDocument(document, {
      viewColumn: options?.openSideBySide
