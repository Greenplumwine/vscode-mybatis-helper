...
export class EnterpriseScanner extends EventEmitter {
  ...
  public static getInstance(
    config?: Partial<EnterpriseScanConfig>,
  ): EnterpriseScanner {
  ...
  public async scan(): Promise<void> {
    ...
    try {
      ...
      // ========== Phase 1: 全面配置发现 ==========
      const {
        configs: mapperScanConfigs,
        sources,
        stats,
      } = await this.configResolver.resolveAllConfigs();

      this.logger?.info(`Configuration discovery completed:`);
      this.logger?.info(
        `  - Found ${mapperScanConfigs.length} @MapperScan configs`,
      );
      this.logger?.info(
      ...
      this.logger?.info(
        `Resolved ${xmlLocations.length} XML location patterns`,
      );
      ...
      if (mapperScanConfigs.length > 0 || xmlLocations.length > 0) {
        ...
        this.logger?.warn(
          "No configuration found, falling back to heuristic scan",
        );
  ...
  private async scanJavaWithConfigs(
    ...
    this.logger?.info(
      `Scanning ${allPackages.size} packages from @MapperScan configs`,
    );
    ...
    this.logger?.info(
      `Found ${uniqueFiles.length} Java files in configured packages`,
    );
  ...
  private async scanHeuristic(): Promise<void> {
    ...
    for (let i = 0; i < javaFiles.length; i += this.config.batchSize) {
      ...
      javaMappers.push(
        ...results.filter((m): m is JavaMapperInfo => m !== null),
      );
    }
