...
export class EnterpriseConfigResolver {
  ...
  private configCache: Map<
    string,
    { configs: MapperScanConfig[]; timestamp: number }
  > = new Map();
  ...
  public async initializeIndexCache(projectRoot: string): Promise<void> {
    ...
    this.logger?.info(
      `Index cache: ${stats.total} entries, ${stats.withMapperScan} with @MapperScan`,
    );
  }
  ...
  public async resolveAllConfigs(): Promise<{
    ...
    for (const result of sourceResults) {
      if (result.status === "fulfilled" && result.value) {
        ...
        if (configs.length > 0) {
          ...
          this.logger?.info(
            `Layer ${layer} (${name}): Found ${configs.length} configs`,
          );
        }
      }
    }
    ...
    if (foundInSource) {
      this.logger?.info(
        "Found @MapperScan in source code, skipping compiled class scanning",
      );
      // 仍然执行，但优先使用缓存
      layerPromises.push(
      ...
      this.logger?.info(
        "No @MapperScan found in source, scanning compiled classes...",
      );
      layerPromises.push(
    ...
    this.logger?.info(
      `Enterprise config resolution completed in ${duration}ms:`,
    );
    this.logger?.info(`  - Total unique configs: ${uniqueConfigs.length}`);
    this.logger?.info(
      `  - Sources: ${sources.map((s) => s.location).join(", ") || "none"}`,
    );
  ...
  private async resolveFromCurrentProject(): Promise<MapperScanConfig[]> {
    ...
    try {
      ...
      for (const pattern of searchPatterns) {
        ...
        this.logger?.debug(
          `Layer 1 pattern "${pattern}": found ${files.length} files`,
        );

        for (const file of files) {
          if (checkedFiles.has(file.fsPath)) {
            continue;
          }
          ...
          // 调试：打印扫描到的文件
          if (
            file.fsPath.includes("ApplicationConfig") ||
            file.fsPath.includes("MapperScan")
          ) {
      ...
      this.logger?.debug(
        `Layer 1 total scanned: ${totalScanned}, found: ${configs.length} configs`,
      );
    } catch (error) {
      ...
      for (const folder of workspaceFolders) {
        ...
        try {
          ...
          if (pomContent.includes("<modules>")) {
            ...
            // 解析子模块
            const moduleMatches = pomContent.matchAll(
              /<module>([^<]+)<\/module>/g,
            );
        ...
        try {
          ...
          if (settingsFile) {
            ...
            if (settingsContent.includes("include")) {
              ...
              // 解析 include 语句
              const includeMatches = settingsContent.matchAll(
                /include\s*['"]([^'"]+)['"]/g,
              );
  ...
  private async resolveFromSubModules(
    multiModuleInfo: MultiModuleInfo,
  ): Promise<MapperScanConfig[]> {
    const configs: MapperScanConfig[] = [];

    this.logger?.debug(
      `Layer 2: Scanning ${multiModuleInfo.modulePaths.length} sub-modules`,
    );
    ...
    for (const modulePath of multiModuleInfo.modulePaths) {
      try {
        ...
        const pattern = new vscode.RelativePattern(
          modulePath,
          "**/*{Config,Configuration,Application,Mybatis}*.java",
        );
        const files = await vscode.workspace.findFiles(
          pattern,
        ...
        this.logger?.debug(
          `Layer 2 module "${path.basename(modulePath)}": found ${files.length} files`,
        );
  ...
  private async resolveFromSourceJars(): Promise<MapperScanConfig[]> {
    ...
    try {
      ...
      const sourceJarPatterns = ["**/*-sources.jar", "**/*-sources.zip"];
      ...
      for (const pattern of sourceJarPatterns) {
        ...
        for (const jar of jars) {
          ...
          if (
            jarName.includes("mybatis") ||
            jarName.includes("mapper") ||
            jarName.includes("dao")
          ) {
  ...
  private async resolveFromCompiledClasses(
    ...
    if (cachedConfigs.length > 0) {
      this.logger?.info(
        `Using ${cachedConfigs.length} configs from index cache`,
      );
    ...
    if (cacheOnly) {
      this.logger?.debug(
        "Cache is empty and cacheOnly=true, skipping compiled class scanning",
      );
      return configs;
    }
    ...
    try {
      ...
      if (!workspaceFolders) {
        this.logger?.debug(
          "No workspace folders found for compiled class scanning",
        );
        return configs;
      }
      ...
      if (!javapAvailable) {
        this.logger?.info(
          "javap not available, skipping compiled class scanning",
        );
        return configs;
      }
      ...
      for (const folder of workspaceFolders) {
        ...
        this.logger?.debug(
          `Checking ${classesPaths.length} compiled classes directories`,
        );
        ...
        for (const classesPath of classesPaths) {
          try {
            ...
            this.logger?.info(
              `Found compiled classes directory: ${classesPath}`,
            );

            // 只查找可能的配置类（性能优化：从300+减少到<20）
            const classFiles = await this.findConfigClassFiles(classesPath);
            this.logger?.info(
              `Found ${classFiles.length} potential config classes in ${classesPath}`,
            );
            ...
            if (layerConfigs.length > 0) {
              ...
              this.logger?.info(
                `Layer 4 (compiled classes): Found ${layerConfigs.length} configs in ${parseTime}ms`,
              );

              // 更新索引缓存
              for (const cfg of layerConfigs) {
                this.logger?.info(
                  `  - ${cfg.sourceFile}: ${cfg.basePackages.join(", ")}`,
                );
              ...
              this.logger?.info(
                `No @MapperScan found in ${classesPath} (${parseTime}ms)`,
              );
            }
          } catch (e) {
            // 目录不存在
            this.logger?.debug(
              `Compiled classes directory not found: ${classesPath}`,
            );
          }
        }
      }
   ...
   */
  private async parseWithWorkerOrMain(
    classFiles: string[],
  ): Promise<MapperScanConfig[]> {
   ...
   */
  private async parseWithWorkerThreads(
    classFiles: string[],
  ): Promise<MapperScanConfig[]> {
    const { Worker } = await import("worker_threads");
    ...
    this.logger?.debug(
      `Processing ${classFiles.length} classes with ${batches.length} workers`,
    );

    // 并行运行所有 Worker
    const workerPromises = batches.map((batch) => {
      return new Promise<{ configs: MapperScanConfig[]; errors: string[] }>(
        (resolve, reject) => {
  ...
  private async resolveFromRuntime(): Promise<MapperScanConfig[]> {
    ...
    try {
      ...
      const mybatisConfigKeys = Object.keys(envVars).filter(
        (key) =>
  ...
  private parseMapperScanFromContent(
    ...
    this.logger?.debug(
      `Parsing @MapperScan from: ${path.basename(sourcePath)}`,
    );
    ...
    for (let i = 0; i < patterns.length; i++) {
      ...
      if (match) {
        this.logger?.debug(
          `  Pattern ${i} matched: ${match[0].substring(0, 50)}...`,
        );
        ...
        if (packages.length > 0) {
          ...
          this.logger?.debug(
            `  Pattern matched but no packages extracted from: ${value}`,
          );
        }
      }
    }
   ...
   */
  private parseMapperScanFromBytecode(
    output: string,
    sourceFile: string,
  ): MapperScanConfig | null {
    // 快速检查是否包含 MapperScan
    ...
    if (basePackages.length > 0) {
      this.logger?.debug(
        `Found @MapperScan with packages: ${basePackages.join(", ")}`,
      );
   ...
   * 从jar包解析注解
   */
  private async parseAnnotationsFromJar(
    jarPath: string,
  ): Promise<MapperScanConfig[]> {
    const configs: MapperScanConfig[] = [];
    ...
    try {
      ...
      const output = execFileSync("jar", ["tf", safeJarPath], {
        encoding: "utf-8",
      });
      const files = output.split("\n");

      // 查找可能的配置类
      const configClasses = files.filter(
        (f: string) =>
      ...
      for (let i = 0; i < limit; i++) {
        ...
        try {
          ...
          const config = this.parseMapperScanFromBytecode(
            javapOutput,
            `${jarPath}!${classFile}`,
          );
          if (config) {
            configs.push(config);
            this.logger?.info(
              `Found @MapperScan in JAR: ${className} -> ${config.basePackages.join(", ")}`,
            );
          }
        } catch (classError) {
          // 单个类解析失败，继续处理其他类
   ...
   * 从class文件解析注解
   */
  private async parseAnnotationsFromClassFile(
    classPath: string,
  ): Promise<MapperScanConfig | null> {
    // 如果 javap 不可用，直接返回 null
    if (!this.isJavapAvailable()) {
      this.logger?.debug(
        "javap not available, skipping class file bytecode parsing",
      );
      return null;
    }
    ...
    try {
      const output = execFileSync("javap", ["-v", safeClassPath], {
        encoding: "utf-8",
        timeout: 2000,
      });
  ...
  private fileCache = new Map<
    string,
    { mtime: number; result: MapperScanConfig | null }
  >();
   ...
   * 批量并行解析 class 文件（带缓存）
   */
  private async parseConfigClassesParallel(
    classFiles: string[],
   ...
   * 带缓存的 class 文件解析
   */
  private async parseAnnotationsFromClassFileCached(
    classPath: string,
  ): Promise<MapperScanConfig | null> {
    try {
      // 检查缓存
  ...
  private async findSubModules(rootPath: string): Promise<string[]> {
    ...
    try {
      ...
      try {
        const gradleSettingsPath = await fs
          .access(settingsPath)
          .then(() => settingsPath)
          .catch(() =>
            fs
    ...
    this.logger?.debug(
      `Found ${modules.length} sub-modules: ${modules.join(", ")}`,
    );
    return modules;
  }
  ...
  private extractPackageNames(value: string, fullContent?: string): string[] {
    ...
    if (value.includes(".class")) {
      ...
      for (const match of classMatches) {
        ...
        // 在内容中查找import语句
        if (fullContent) {
          const importMatch = fullContent.match(
            new RegExp(`import\s+([\w.]+)\.${className};`),
          );
          if (importMatch) {
