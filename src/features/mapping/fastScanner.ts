...
export class FastScanner extends EventEmitter {
  ...
  public async scan(): Promise<void> {
    ...
    try {
      ...
      this.logger?.info(
        `Found ${mapperScanConfigs.length} @MapperScan configs, ${xmlLocations.length} XML locations`,
      );
      ...
      this.logger?.info(
        `Scan completed in ${duration}ms: ${stats.total} mappings, ${stats.withXml} with XML, ${stats.totalMethods} methods`,
      );
  ...
  private async getMapperScanConfigsFast(): Promise<MapperScanConfig[]> {
    ...
    try {
      ...
      for (const pattern of configPatterns) {
        ...
        for (const file of files) {
          ...
          if (config) {
            configs.push(config);
            this.logger?.info(
              `Found @MapperScan in ${file.fsPath}: ${config.basePackages.join(", ")}`,
            );
          }
        }
        ...
        if (configs.length >= 3) {
          this.logger?.debug(
            `Found ${configs.length} @MapperScan configs early in phase 1`,
          );
          return configs;
        }
      }
      ...
      if (configs.length > 0) {
        this.logger?.debug(
          `Found ${configs.length} @MapperScan configs in phase 1`,
        );
        return configs;
      }
      ...
      // ========== 策略2: 全局文本搜索（更彻底但较慢）==========
      this.logger?.info(
        "Phase 1 found no @MapperScan, trying global text search...",
      );
      ...
      for (
        let i = 0;
        i < allJavaFiles.length && configs.length === 0;
        i += batchSize
      ) {
        ...
        for (const config of batchResults) {
          if (config) {
            configs.push(config);
            this.logger?.info(
              `Found @MapperScan via global search in ${config.sourceFile}: ${config.basePackages.join(", ")}`,
            );
          }
        }
        ...
        if (i % 100 === 0) {
          this.logger?.debug(
            `Checked ${Math.min(i + batchSize, allJavaFiles.length)}/${allJavaFiles.length} files...`,
          );
        }
      }
      ...
      this.logger?.debug(
        `Found ${configs.length} @MapperScan configs in ${Date.now() - startTime}ms`,
      );
      return configs;
    } catch (error) {
  ...
  private parseMapperScanFromContent(
    ...
    const mapperScanRegex =
      /@MapperScan\s*\(\s*(?:(?:value|basePackages)\s*=\s*)?([\s\S]*?)\s*\)/;
  ...
  private async scanWithConfig(
    ...
    this.logger?.info(
      `Scanned ${xmlMappers.length} XML mappers, ${javaMappers.length} Java mappers in ${Date.now() - startTime}ms`,
    );
   ...
   * 根据配置位置扫描 XML
   */
  private async scanXmlWithLocations(
    locations: string[],
  ): Promise<XmlMapperInfo[]> {
    ...
    for (const pattern of locations) {
      try {
        ...
        for (const file of files) {
          if (scannedPaths.has(file.fsPath)) {
            continue;
          }
          if (mappers.length >= this.config.maxXmlFiles) {
            break;
          }
   ...
   */
  private async scanJavaWithConfigs(
    configs: MapperScanConfig[],
  ): Promise<JavaMapperInfo[]> {
  ...
  private async scanHeuristic(): Promise<void> {
    ...
    const javaPatterns = ["**/*Mapper.java", "**/*Dao.java"];
  ...
  private async findFilesWithLimit(
    patterns: string[],
    limit: number,
  ): Promise<vscode.Uri[]> {
   ...
   */
  private async parseJavaMapperFast(
    filePath: string,
