  ...
  implements vscode.CompletionItemProvider
{
  ...
  constructor(
    ...
    // 初始化上下文构建器
    this.contextBuilder = new CompletionContextBuilder(javaParser, xmlParser);
  ...
  registerStrategy(strategy: CompletionStrategy): void {
    ...
    this.logger.debug(
      `Registered completion strategy: ${strategy.name} (priority: ${strategy.priority})`,
    );
  }
  ...
  async provideCompletionItems(
    ...
    // 入口日志
    this.logger.debug(
      `[UnifiedCompletion] triggered by: ${context.triggerCharacter}, language: ${document.languageId}`,
    );
  ...
  private async selectStrategy(
    ...
    for (const strategy of this.strategies) {
      try {
        // 检查策略是否匹配
        const canComplete = await Promise.resolve(
          strategy.canComplete(context),
        );
        ...
        this.logger.warn(
          `Strategy ${strategy.name} canComplete check failed:`,
          error,
        );
        // 继续检查下一个策略
      }
  ...
  private registerDefaultStrategies(): void {
    ...
    this.registerStrategy(
      new PropertyStrategy(this.javaParser, this.getMaxPropertyDepth()),
    );
    ...
    this.logger.info(
      `Registered ${this.strategies.length} default completion strategies`,
    );
  }
