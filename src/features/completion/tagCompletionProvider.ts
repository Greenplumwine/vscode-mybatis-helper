...
export class TagCompletionProvider implements vscode.CompletionItemProvider {
  /** 触发字符 */
  static readonly triggerCharacters = ['<', ' ', '"', "'", 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'] as const;
  ...
  async provideCompletionItems(
    ...
    this.logger.debug(`Tag completion triggered by: ${triggerChar}, context:`, xmlContext);
    ...
    try {
      ...
      if (xmlContext.isInTag) {
        ...
        if (ltIndex !== -1) {
          ...
          this.logger.debug(`Tag completion range: ${range.start.character}-${range.end.character}, text: "${lineText.substring(ltIndex, endPos)}"`);
        }
        ...
        switch (triggerChar) {
          ...
          case "'":
            // 引号触发：提供属性值补全
            if (xmlContext.isInAttributeValue && xmlContext.currentTag && xmlContext.currentAttribute) {
          ...
          default:
            ...
            if (items.length > 0) {
              const first = items[0];
              this.logger.debug(`First item: label=${first.label}, kind=${first.kind}, insertText=${typeof first.insertText === 'string' ? first.insertText : '(SnippetString)'}`);
              const rangeStr = first.range && 'start' in first.range ? `${first.range.start.character}-${first.range.end.character}` : 'undefined';
  ...
  private getBuiltinHierarchy(): Map<string, TagHierarchy> {
    ...
    hierarchy.set('mapper', {
      parentTag: 'root',
      allowedChildren: ['cache-ref', 'cache', 'resultMap', 'parameterMap', 'sql', 'insert', 'update', 'delete', 'select'],
    ...
    hierarchy.set('select', {
      parentTag: 'mapper',
      allowedChildren: ['include', 'trim', 'where', 'set', 'foreach', 'choose', 'if', 'bind'],
      allowedAttributes: ['id', 'resultType', 'resultMap', 'parameterType', 'useCache', 'flushCache', 'timeout', 'fetchSize', 'statementType', 'resultOrdered', 'resultSetType']
    });
    
    hierarchy.set('insert', {
      parentTag: 'mapper',
      allowedChildren: ['selectKey', 'include', 'trim', 'where', 'set', 'foreach', 'choose', 'if', 'bind'],
      allowedAttributes: ['id', 'parameterType', 'useGeneratedKeys', 'keyProperty', 'keyColumn', 'timeout', 'flushCache', 'statementType']
    });
    
    hierarchy.set('update', {
      parentTag: 'mapper',
      allowedChildren: ['include', 'trim', 'where', 'set', 'foreach', 'choose', 'if', 'bind'],
      allowedAttributes: ['id', 'parameterType', 'timeout', 'flushCache', 'statementType']
    });
    
    hierarchy.set('delete', {
      parentTag: 'mapper',
      allowedChildren: ['include', 'trim', 'where', 'foreach', 'choose', 'if', 'bind'],
      allowedAttributes: ['id', 'parameterType', 'timeout', 'flushCache', 'statementType']
    });
    ...
    hierarchy.set('if', {
      parentTag: '*',
      allowedChildren: ['include', 'trim', 'where', 'set', 'foreach', 'choose', 'bind', 'if'],
  ...
  private provideTagNames(
    ...
    if (!parentTag) {
      ...
      const items = [this.createTagItem('mapper', 'Root element', range, inputPrefix)];
      return filterPrefix 
        ? items.filter(item => item.label.toString().toLowerCase().startsWith(filterPrefix.toLowerCase()))
        : items;
    }
    ...
    if (filterPrefix) {
      items = items.filter(item => 
        item.label.toString().toLowerCase().startsWith(filterPrefix.toLowerCase())
      );
    }
  ...
  private parseXmlContext(
    ...
    this.logger.debug(`Cursor context: line="${lineText}" | after="${afterCursor.substring(0, 20)}"`);
    ...
    while ((match = tagPattern.exec(contentBeforeCursor)) !== null) {
      if (match.index >= offset) break; // 跳过光标后的内容
      ...
      // DEBUG: 输出匹配到的标签
      this.logger.debug(`Matched tag: "${fullMatch}", name=${tagName}, isClosing=${isClosing}, isSelfClosing=${isSelfClosing}`);
    ...
    // DEBUG: 输出完整标签栈
    this.logger.debug(`Tag stack: [${tagStack.join(', ')}], parentTag: ${context.parentTag}`);
   ...
   * 创建标签补全项
   */
  private createTagItem(tagName: string, detail?: string, range?: vscode.Range, inputPrefix?: string): vscode.CompletionItem {
   ...
   * 创建属性补全项
   */
  private createAttributeItem(attribute: string, tagName: string): vscode.CompletionItem {
