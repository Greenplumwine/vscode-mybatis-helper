...
export class CompletionContextBuilder {
  ...
  private async enrichMyBatisContext(
    ...
    try {
      ...
      const foreachContext =
        this.xmlParser.findForeachContext?.(
          document.getText(),
          position.line,
        ) ?? undefined;
      ...
      if (currentMethod) {
        javaMethod = await this.findJavaMethod(
          document.fileName,
          currentMethod.id,
        );
      }
   ...
   */
  private async parseXmlMapperWithFallback(
    document: vscode.TextDocument,
  ...
  private parseXmlSimple(content: string, filePath: string): XmlMapperInfo {
    ...
    const methodPattern =
      /<(select|insert|update|delete)\s+[^>]*id=["']([^"']+)["'][^>]*>/gi;
    let match;
    ...
    while ((match = methodPattern.exec(content)) !== null) {
      ...
      if (endTagPos !== -1) {
        ...
        const nextTagMatch = content.match(
          /<\/(select|insert|update|delete)>/i,
        );
        if (
          nextTagMatch &&
          nextTagMatch.index &&
          nextTagMatch.index > match.index
        ) {
          const beforeNextTag = content.substring(
            0,
            nextTagMatch.index + nextTagMatch[0].length,
          );
  ...
  private async findJavaMethod(
    ...
    try {
      ...
      this.logger.debug(
        `Parse result: ${result ? `found ${result.name}` : "null/undefined"}`,
      );
      return result ?? undefined;
    } catch (error) {
      this.logger.debug(
        `Failed to parse Java method ${methodName} in ${javaPath}:`,
        error,
      );
      return undefined;
    }
