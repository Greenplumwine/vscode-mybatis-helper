...
export class TypeHandlerStrategy extends BaseCompletionStrategy {
  ...
  async provideCompletionItems(
    ...
    // 过滤 TypeHandler
    const filtered = TypeHandlerStrategy.HANDLERS.filter(handler => 
    ...
    // 创建补全项
    return filtered.map((handler, index) => this.createHandlerItem(handler, index));
  }
   ...
   */
  private createHandlerItem(handler: TypeHandlerInfo, index: number): vscode.CompletionItem {
    // 构建文档
   ...
   * @returns Markdown 文档
   */
  private buildHandlerDocumentation(handler: TypeHandlerInfo): vscode.MarkdownString {
