...
export class TypeStrategy extends BaseCompletionStrategy {
  ...
  private filterAndSortClasses(
    ...
    return classes
      .filter((cls) => {
        // 简单名或全限定名包含输入内容
        return (
          cls.simpleName.toLowerCase().includes(lowerPartial) ||
      ...
      .sort((a, b) => {
        ...
        // 3. 常用类型优先
        const aCommon = TypeStrategy.COMMON_TYPES.includes(
          a.fullyQualifiedName,
        );
        const bCommon = TypeStrategy.COMMON_TYPES.includes(
  ...
  private createTypeItems(classes: ClassInfo[]): vscode.CompletionItem[] {
    return classes.map((cls, index) => {
      const isCommon = TypeStrategy.COMMON_TYPES.includes(
  ...
  private createCommonTypeItems(partial: string): vscode.CompletionItem[] {
    ...
    return TypeStrategy.COMMON_TYPES.filter((type) =>
      type.toLowerCase().includes(lowerPartial),
    ).map((type, index) => {
   ...
   * @returns Markdown 文档
   */
  private buildTypeDocumentation(
    cls: ClassInfo,
    isCommon: boolean,
