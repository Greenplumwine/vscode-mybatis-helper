...
const BUILTIN_RULES: SQLInterceptorRule[] = [
  {
    ...
    enabled: true,
    description:
      'Universal MyBatis log matcher - matches any format with Preparing:/Parameters: keywords. Supports: "Preparing: SELECT...", "Parameters: admin(String)", etc.',
    ...
    executionTimeExtractRegex:
      "(?:<==\\s*)?(?:[Tt]otal|[Ee]xecuted|[Tt]ime)[:：]\\s*(\\d+)",
  ...
  },
  {
    ...
    enabled: true,
    description:
      "MyBatis SqlSession debug logs with class name prefix like c.r.s.m.S.selectXxx or o.a.i.s.SqlSession.selectXxx",
    // 匹配常见的类名前缀格式：c.r.s.m.S.selectXxx, o.a.i.l.s.d.S.selectXxx 等
    lineMatchRegex:
      "(?:[a-z]\\.[a-z]\\.[a-z]\\.[a-z]\\.[A-Z]|[A-Z][a-z]+Mapper\\.|[Ss]ql[Ss]ession[^\\s]*)\\s+(?:preparing|parameters)",
...
export class SQLInterceptorService {
  ...
  private loadDefaultConfig(): SQLInterceptorConfig {
    ...
    return {
      ...
      maxHistorySize: workspaceConfig.get(
        "sqlInterceptor.maxHistorySize",
        CACHE_LIMITS.DEFAULT_MAX_HISTORY,
      ),
      showExecutionTime: workspaceConfig.get(
        "sqlInterceptor.showExecutionTime",
        true,
      ),
      databaseType: workspaceConfig.get<DatabaseType>(
        "databaseType",
        DatabaseType.MYSQL,
      ),
      ...
      autoScrollBehavior: workspaceConfig.get(
        "sqlInterceptor.autoScrollBehavior",
        "onlyWhenNotInteracting",
      ),
    };
  }
  ...
  public async initialize(): Promise<void> {
    ...
    if (this.config.enabled && this.config.autoStart) {
      ...
      // 显示提示，告知用户需要手动启动
      logger.info(
        "[SQLInterceptor] Auto-start is disabled, waiting for manual start",
      );
      // 延迟显示，避免启动时消息过多
      setTimeout(() => {
        if (!this.isEnabled) {
  ...
  private setupAutoListenMode(): void {
    // 读取 Java 插件的配置
    const javaDebugConfig = vscode.workspace.getConfiguration(
      "java.debug.settings",
    );
    const consoleType = javaDebugConfig.get<string>(
      "console",
      "internalConsole",
    );
    ...
    switch (consoleType) {
      case "internalConsole":
        ...
        this.listenToDebugConsole();
        logger.info(
          "[SQLInterceptor] Auto-configured: Listening to Debug Console",
        );
        break;
      ...
      case "externalTerminal":
        // Java 在外部终端中运行 - 无法监听
        logger.warn(
          "[SQLInterceptor] Warning: Java is configured to run in external terminal, which cannot be monitored",
        );
      ...
      default:
        ...
        logger.warn(
          `[SQLInterceptor] Unknown console type: ${consoleType}, defaulting to auto (Debug Console)`,
        );
        this.listenToDebugConsole();
    }
  }
  ...
  private listenToDebugConsole(): void {
    ...
    const trackerFactory: vscode.DebugAdapterTrackerFactory = {
      createDebugAdapterTracker: (session: vscode.DebugSession) => {
        logger.debug(
          `[SQLInterceptor] Creating tracker for session: ${session.name} (type: ${session.type})`,
        );
        ...
        return {
              ...
              if (body && body.output) {
                ...
                if (
                  ...
                  for (const line of lines) {
                    if (line.trim()) {
                      logger.debug(
                        `[SQLInterceptor] Debug output: ${line.substring(0, THRESHOLDS.LOG_TRUNCATE_LONG)}...`,
                      );
          ...
          onError: (error: Error) => {
            logger.error(
              `[SQLInterceptor] Debug adapter error: ${error.message}`,
            );
          },
          ...
          onExit: (code: number | undefined, signal: string | undefined) => {
            logger.debug(
              `[SQLInterceptor] Debug adapter exited: code=${code}, signal=${signal}`,
            );
          },
        };
      },
    ...
    // 注册 tracker 工厂
    const trackerDisposal = vscode.debug.registerDebugAdapterTrackerFactory(
      "*",
      trackerFactory,
    );
    ...
    // 监听调试会话开始
    const startSessionDisposal = vscode.debug.onDidStartDebugSession(
      (session) => {
        logger.info(
          `[SQLInterceptor] Debug session started: ${session.name} (type: ${session.type})`,
        );
      },
    );
    ...
    // 监听调试会话结束
    const terminateSessionDisposal = vscode.debug.onDidTerminateDebugSession(
      (session) => {
        logger.info(
          `[SQLInterceptor] Debug session terminated: ${session.name}`,
        );
        // 清理当前查询
        this.pendingQuery = null;
      },
  ...
  private listenToTerminal(): void {
    logger.info(
      "[SQLInterceptor] Using Shell Integration API to listen terminal output",
    );
    ...
    if (vscode.window.onDidStartTerminalShellExecution) {
      ...
      vscode.window.onDidStartTerminalShellExecution(
        async (event: any) => {
          ...
          logger.debug(
            `[SQLInterceptor] Terminal shell execution started: ${terminal.name}`,
          );

          // 检查终端名称是否符合过滤条件
          if (
            this.config.terminalFilter &&
            !terminal.name.includes(this.config.terminalFilter)
          ) {
            logger.debug(
              `[SQLInterceptor] Terminal filtered out: ${terminal.name}`,
            );
            return;
          }
          ...
          try {
            ...
            if (execution.read) {
              ...
              logger.debug(
                `[SQLInterceptor] Terminal execution completed, total output length: ${output.length}`,
              );
            }
          } catch (error) {
            logger.error(
              "[SQLInterceptor] Error reading terminal output:",
              error as Error,
            );
          }
        },
      ...
      logger.info(
        "[SQLInterceptor] Terminal shell execution listener registered successfully",
      );
    } else {
      logger.warn(
        "[SQLInterceptor] Shell Integration API not available in this VS Code version",
      );
    }
  ...
  public processLogLine(line: string, source: "debug" | "terminal"): void {
    ...
    try {
      ...
      logger.debug(
        `[SQLInterceptor] processLogLine: parsed type=${parsed.type}, line=${line.substring(0, 80)}...`,
      );
      ...
      logger.error(
        "[SQLInterceptor] Error processing log line:",
        error as Error,
      );
    }
  }
  ...
  private flushLineBuffer(source: "debug" | "terminal"): void {
    ...
    // 构建解析结果并处理
    if (sql) {
      this.handleParsedLog(
        { type: "sql", content: bufferText, ruleName: rule.name, sql },
        source,
      );
    }
  ...
  private parseLogLine(
    ...
    for (const rule of rules) {
      ...
      logger.debug(
        `[SQLInterceptor] Rule '${rule.name}' matched line: ${line.substring(0, 60)}...`,
      );
      ...
      if (sqlMatch && sqlMatch[1]) {
        logger.debug(
          `[SQLInterceptor] Extracted SQL: ${sqlMatch[1].substring(0, 50)}...`,
        );
        return {
          type: "sql",
          content: line,
      ...
      if (rule.parametersExtractRegex) {
        ...
        if (paramMatch && paramMatch[1]) {
          logger.debug(
            `[SQLInterceptor] Extracted parameters: ${paramMatch[1]}`,
          );
          return {
            type: "parameters",
            content: line,
      ...
      if (rule.executionTimeExtractRegex) {
        ...
        if (timeMatch && timeMatch[1]) {
          logger.debug(
            `[SQLInterceptor] Extracted execution time: ${timeMatch[1]}ms`,
          );
          return {
            type: "executionTime",
            content: line,
   ...
   * 处理解析后的日志
   */
  private handleParsedLog(
    parsed: ParsedLogLine,
    source: "debug" | "terminal",
  ): void {
    logger.debug(
      `[SQLInterceptor] handleParsedLog: type=${parsed.type}, rule=${parsed.ruleName}, hasPending=${!!this.pendingQuery}`,
    );
    ...
    switch (parsed.type) {
      case "sql":
        ...
        if (
          sqlKey === this.lastProcessedSQL &&
          now - this.lastProcessedTime < TIME.DUPLICATE_CHECK_WINDOW
        ) {
        ...
        logger.debug(
          `[SQLInterceptor] New SQL query started: ${parsed.sql?.substring(0, 50)}...`,
        );

        // 设置超时，如果在指定时间内没有收到参数，则直接保存
        this.pendingTimer = setTimeout(() => {
          logger.debug(
            `[SQLInterceptor] Query timeout, finalizing without parameters`,
          );
          this.finalizePendingQuery();
        }, 5000);
        break;
      ...
      case "parameters":
        if (this.pendingQuery) {
          logger.debug(
            `[SQLInterceptor] Processing parameters: ${parsed.parameters}`,
          );
          ...
          if (this.pendingQuery.rawSQL) {
            ...
            logger.debug(
              `[SQLInterceptor] Built full SQL: ${this.pendingQuery.fullSQL?.substring(0, 50)}...`,
            );
          }
        } else {
          logger.warn(
            `[SQLInterceptor] Received parameters but no pending query!`,
          );
        }
        break;
      ...
      case "executionTime":
        if (this.pendingQuery) {
          ...
          logger.debug(
            `[SQLInterceptor] Execution time: ${parsed.executionTime}ms`,
          );
          // 收到执行时间， finalize 查询
          this.finalizePendingQuery();
        }
  ...
  private finalizePendingQuery(): void {
    ...
    logger.debug(
      `[SQLInterceptor] Finalizing query with ${paramCount} parameters`,
    );
  ...
  private getCachedRegex(pattern: string): RegExp {
    ...
    if (!regex) {
      try {
        ...
        logger.error(
          `[SQLInterceptor] Invalid regex pattern: ${pattern}`,
          error as Error,
        );
        // 返回永不匹配的正则
        return /(?!)/;
