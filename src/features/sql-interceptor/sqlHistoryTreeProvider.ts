...
export class SQLHistoryTreeProvider implements vscode.TreeDataProvider<SQLTreeItem> {
  private _onDidChangeTreeData: vscode.EventEmitter<
    SQLTreeItem | undefined | null | void
  > = new vscode.EventEmitter<SQLTreeItem | undefined | null | void>();
  ...
  private getSQLHistoryItems(): SQLTreeItem[] {
    ...
    return history.map((query, index) => {
      // 生成简短标签（SQL 前 N 个字符）
      const shortSQL = this.truncateSQL(
        query.formattedSQL || query.rawSQL || "",
        THRESHOLDS.SQL_DISPLAY_TRUNCATE,
      );
  ...
  private getQueryDetailItems(query: SQLQueryRecord): SQLTreeItem[] {
    ...
    if (query.parameters && query.parameters.length > 0) {
      items.push(
        new SQLTreeItem(
          ...
          vscode.l10n.t("sqlTree.parameters", {
            count: query.parameters.length,
          }),
          vscode.TreeItemCollapsibleState.Collapsed,
          query,
          "sqlParameters",
          vscode.l10n.t("sqlTree.parametersTooltip", {
            count: query.parameters.length,
          }),
        ),
      );
    ...
    if (query.executionTime !== undefined) {
      const timeLabel = vscode.l10n.t("sqlTree.executionTime", {
      ...
      items.push(
        new SQLTreeItem(
          ...
          query,
          "sqlExecutionTime",
          vscode.l10n.t("sqlTree.executionTimeTooltip", {
            time: query.executionTime,
          }),
        ),
      );
  ...
  private getOperationType(sql: string): string {
    ...
    const match = trimmed.match(
      /^(SELECT|INSERT|UPDATE|DELETE|MERGE|CREATE|DROP|ALTER|TRUNCATE|EXEC|CALL)\b/,
    );
