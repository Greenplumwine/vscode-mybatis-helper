...
function outputValidationReport(
    ...
    outputChannel.appendLine(`${vscode.l10n.t('validation.timestamp')}: ${timestamp}`);
    outputChannel.appendLine(`${vscode.l10n.t('validation.status')}: ${result.valid ? vscode.l10n.t('validation.statusValid') : vscode.l10n.t('validation.statusInvalid')}`);
    outputChannel.appendLine('');

    // Configuration file path
    const configPath = getSettingsJsonPath();
    if (configPath) {
        outputChannel.appendLine(`${vscode.l10n.t('validation.configFile')}: ${configPath}`);
        outputChannel.appendLine('');
    }

    // Issues summary
    outputChannel.appendLine(`${vscode.l10n.t('validation.issuesFound')}: ${result.issues.length}`);
    outputChannel.appendLine(`  - ${vscode.l10n.t('validation.errors')}: ${errors.length}`);
    outputChannel.appendLine(`  - ${vscode.l10n.t('validation.warnings')}: ${warnings.length}`);
    outputChannel.appendLine(`  - ${vscode.l10n.t('validation.info')}: ${infos.length}`);
    outputChannel.appendLine('');
    ...
    if (result.issues.length > 0) {
        ...
        for (const issue of result.issues) {
            ...
            outputChannel.appendLine(`  ${vscode.l10n.t('validation.suggestion')}: ${issue.suggestion}`);
            outputChannel.appendLine('');
        }
    }
