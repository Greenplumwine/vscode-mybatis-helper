...
async function diagnoseEnvironment(
  ...
  outputChannel.appendLine(
    `  ${vscode.l10n.t("diagnostics.vscode")}: ${vscodeVersion}`,
  );
  ...
  if (javaExt) {
    ...
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.javaExt")}: ${status}`,
    );
  } else {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.javaExt")}: ${vscode.l10n.t("diagnostics.status.notInstalled")}`,
    );
  }
  ...
  outputChannel.appendLine(
    `  ${vscode.l10n.t("diagnostics.os")}: ${osName} (${platform})`,
  );

  // Extension version
  const ext = vscode.extensions.getExtension(
    "Greenplumwine.mybatis-helper-vscode",
  );
  if (ext) {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.extVersion")}: ${ext.packageJSON.version}`,
    );
  }
...
async function diagnoseProject(
  ...
  outputChannel.appendLine(
    `  ${vscode.l10n.t("diagnostics.workspace")}: ${workspaceRoot}`,
  );
  ...
  if (buildTool) {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.buildTool")}: ${buildTool}`,
    );
  } else {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.buildTool")}: ${vscode.l10n.t("diagnostics.unknown")}`,
    );
  }
  ...
  outputChannel.appendLine(
    `  ${vscode.l10n.t("diagnostics.javaFiles")}: ${javaFiles.length}`,
  );
  ...
  outputChannel.appendLine(
    `  ${vscode.l10n.t("diagnostics.xmlFiles")}: ${xmlFiles.length}`,
  );
 ...
 * Section 3: Mapper Mappings
 */
async function diagnoseMappings(
  outputChannel: vscode.OutputChannel,
): Promise<void> {
  ...
  try {
    ...
    // Total mappings
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.totalMappings")}: ${stats.total}`,
    );
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.withXml")}: ${stats.withXml}`,
    );
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.totalMethods")}: ${stats.totalMethods}`,
    );
    ...
    if (diagnostics.indexSizes) {
      ...
      outputChannel.appendLine(
        `    - ${vscode.l10n.t("diagnostics.namespaceIndex")}: ${diagnostics.indexSizes.namespace || 0}`,
      );
      outputChannel.appendLine(
        `    - ${vscode.l10n.t("diagnostics.javaPathIndex")}: ${diagnostics.indexSizes.javaPath || 0}`,
      );
      outputChannel.appendLine(
        `    - ${vscode.l10n.t("diagnostics.xmlPathIndex")}: ${diagnostics.indexSizes.xmlPath || 0}`,
      );
    }
    ...
    for (const javaFile of javaFiles.slice(0, 100)) {
      // Limit to first 100
      ...
      if (!mapping || !mapping.xmlPath) {
        ...
        try {
          ...
          const hasMyBatis =
            /@Mapper|@Select|@Insert|@Update|@Delete|import\s+org\.apache\.ibatis/.test(
              content,
            );
    ...
    if (unmappedJava.length > 0) {
      outputChannel.appendLine(
        `  ${vscode.l10n.t("diagnostics.unmappedJava")}: ${unmappedJava.length}`,
      );
      outputChannel.appendLine(
        `    ${vscode.l10n.t("diagnostics.files")}: ${unmappedJava.slice(0, 10).join(", ")}${unmappedJava.length > 10 ? "..." : ""}`,
      );
      outputChannel.appendLine(
        `    ${vscode.l10n.t("diagnostics.suggestion")}: ${vscode.l10n.t("diagnostics.checkXmlDirs")}`,
      );
    }
    ...
    for (const xmlFile of xmlFiles.slice(0, 100)) {
      // Limit to first 100
      ...
      if (!mapping) {
        ...
        try {
          ...
          if (
            content.includes("<!DOCTYPE mapper") ||
    ...
    if (unmappedXml.length > 0) {
      outputChannel.appendLine(
        `  ${vscode.l10n.t("diagnostics.unmappedXml")}: ${unmappedXml.length}`,
      );
      outputChannel.appendLine(
        `    ${vscode.l10n.t("diagnostics.files")}: ${unmappedXml.slice(0, 10).join(", ")}${unmappedXml.length > 10 ? "..." : ""}`,
      );
      outputChannel.appendLine(
        `    ${vscode.l10n.t("diagnostics.suggestion")}: ${vscode.l10n.t("diagnostics.checkJavaFiles")}`,
      );
    }
  } catch (error) {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.mappingError")}: ${error}`,
    );
  }
 ...
 * Section 4: SQL Interceptor
 */
async function diagnoseSqlInterceptor(
  outputChannel: vscode.OutputChannel,
): Promise<void> {
  ...
  try {
    ...
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.status")}: ${status}`,
    );

    // Mode
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.mode")}: ${config.listenMode}`,
    );

    // History entries
    const history = interceptor.getHistory();
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.historyEntries")}: ${history.length}`,
    );

    // Auto-start
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.autoStart")}: ${config.autoStart ? vscode.l10n.t("diagnostics.enabled") : vscode.l10n.t("diagnostics.disabled")}`,
    );

    if (!interceptor.isRunning) {
      outputChannel.appendLine(
        `  ${vscode.l10n.t("diagnostics.suggestion")}: ${vscode.l10n.t("diagnostics.checkSqlConfig")}`,
      );
    }
  } catch (error) {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.sqlError")}: ${error}`,
    );
  }
...
async function diagnoseConfiguration(
  ...
  try {
    ...
    if (result.valid && result.issues.length === 0) {
      ...
      outputChannel.appendLine(
        `  ${vscode.l10n.t("diagnostics.configIssues")}: ${errors.length} ${vscode.l10n.t("diagnostics.errors")}, ${warnings.length} ${vscode.l10n.t("diagnostics.warnings")}`,
      );
      ...
      for (const issue of result.issues.slice(0, 5)) {
        ...
        outputChannel.appendLine(
          `    ${icon} ${issue.configPath}: ${issue.message}`,
        );
      }
    ...
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.configError")}: ${error}`,
    );
  }
...
async function generateRecommendations(
  ...
  try {
    ...
    if (stats.total === 0) {
      ...
      recommendations.push(
        vscode.l10n.t(
          "diagnostics.rec.someUnmapped",
          String(stats.total - stats.withXml),
        ),
      );
    }
  } catch {
  ...
  if (recommendations.length === 0) {
    outputChannel.appendLine(
      `  ${vscode.l10n.t("diagnostics.noRecommendations")}`,
    );
  } else {
