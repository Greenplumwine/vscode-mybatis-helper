 ...
 * Step 1: Project Type Detection
 */
async function step1ProjectType(): Promise<'maven' | 'gradle' | 'other' | undefined> {
    ...
    const items: vscode.QuickPickItem[] = [
        {
            ...
            description: detectedType === 'maven' ? vscode.l10n.t('wizard.projectType.detected') : '',
            picked: detectedType === 'maven'
        },
        {
            label: vscode.l10n.t('wizard.projectType.gradle'),
            description: detectedType === 'gradle' ? vscode.l10n.t('wizard.projectType.detected') : '',
            picked: detectedType === 'gradle'
        },
        {
            label: vscode.l10n.t('wizard.projectType.other'),
            description: detectedType === 'other' ? vscode.l10n.t('wizard.projectType.detected') : ''
        }
    ];
...
async function detectProjectType(): Promise<'maven' | 'gradle' | 'other'> {
    ...
    for (const folder of workspaceFolders) {
        ...
        try {
            const gradleExists = await fs.access(path.join(folder.uri.fsPath, 'build.gradle'))
                .then(() => true)
                .catch(() => fs.access(path.join(folder.uri.fsPath, 'build.gradle.kts'))
                    .then(() => true)
                    .catch(() => false)
                );
            if (gradleExists) {
 ...
 * Step 2: XML Directories Configuration
 */
async function step2XmlDirectories(projectType: 'maven' | 'gradle' | 'other'): Promise<string[] | undefined> {
    ...
    while (addingDirectories) {
        const currentDirsText = directories.length > 0
            ? directories.join(', ')
 ...
 */
function getDefaultXmlDirectories(projectType: 'maven' | 'gradle' | 'other'): string[] {
    switch (projectType) {
        case 'maven':
 ...
 */
async function step3NamingConvention(): Promise<{ convention: string; customPattern?: string } | undefined> {
    const items: vscode.QuickPickItem[] = [
        {
 ...
 * Step 4: SQL Interception Mode
 */
async function step4SqlInterceptionMode(): Promise<'auto' | 'debugConsole' | 'terminal' | undefined> {
    const items: vscode.QuickPickItem[] = [
        {
...
async function saveConfiguration(state: WizardState): Promise<void> {
    ...
    const nameMatchingRules = buildNameMatchingRules(state.namingConvention, state.customPattern);
 ...
 */
function buildNameMatchingRules(convention: string, customPattern?: string): Array<{
    name: string;
    enabled: boolean;
    javaPattern: string;
    xmlPattern: string;
