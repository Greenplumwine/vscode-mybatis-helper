...
export async function showPerformanceStatsCommand(): Promise<void> {
  try {
    ...
    vscode.window.showInformationMessage(vscode.l10n.t('performance.statsDisplayed'));
  } catch (error) {
    vscode.window.showErrorMessage(vscode.l10n.t('performance.statsFailed', { error: String(error) }));
