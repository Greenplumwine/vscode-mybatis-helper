...
export class CreateMapperXmlCommand {
    ...
    try {
      ...
      if (!targetUri) {
        ...
        if (!editor || !editor.document.fileName.endsWith('.java')) {
          vscode.window.showWarningMessage(vscode.l10n.t('createMapperXml.noJavaFile'));
          return;
        }
      ...
      vscode.window.showErrorMessage(
        vscode.l10n.t('createMapperXml.failed', { error: error instanceof Error ? error.message : 'Unknown error' })
      );
    }
  }
  ...
  private async createMapperXml(javaPath: string): Promise<void> {
    ...
    try {
      ...
      const overwrite = await vscode.window.showQuickPick(
        [vscode.l10n.t('quickPick.overwrite'), vscode.l10n.t('quickPick.cancel')],
        { placeHolder: vscode.l10n.t('createMapperXml.fileExists', { fileName: path.basename(xmlPath) }) }
      );
    ...
    vscode.window.showInformationMessage(
      vscode.l10n.t('createMapperXml.created', { fileName: path.basename(xmlPath) })
    );
  }
  ...
  private async determineXmlPath(javaPath: string): Promise<string> {
    ...
    if (customDir) {
      const workspaceFolder = vscode.workspace.getWorkspaceFolder(vscode.Uri.file(javaPath));
      if (workspaceFolder) {
  ...
  private parseJavaMethods(content: string): JavaMethod[] {
    ...
    // 方法定义正则
    const methodPattern = /(\w+(?:<[^>]+>)?)\s+(\w+)\s*\(([^)]*)\)\s*(?:throws\s+[\w,\s]+)?\s*;/g;
...
`;
  ...
  private generateMethodTag(method: JavaMethod): string {
    ...
    // 参数类型
    const paramType = method.parameters.length > 0 ? ` parameterType="${this.extractSimpleName(method.parameters[0].type)}"` : '';
    
    // 结果类型
    let resultType = '';
    if (tagType === 'select' && !method.returnType.toLowerCase().includes('void')) {
      const simpleReturn = this.extractSimpleName(method.returnType);
      if (!simpleReturn.toLowerCase().includes('list') && 
    ...
    if (method.parameters.length > 0) {
      ...
      for (const param of method.parameters) {
        lines.push(`      - ${param.name}: ${this.extractSimpleName(param.type)}`);
      }
    }
