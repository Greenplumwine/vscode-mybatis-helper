...
export class GenerateXmlMethodCommand {
   ...
   * @param args - 命令参数
   */
  async execute(args?: { javaPath?: string; methodName?: string }): Promise<void> {
    try {
      const editor = vscode.window.activeTextEditor;
      if (!editor) {
        vscode.window.showWarningMessage(vscode.l10n.t('generateXmlMethod.noActiveEditor'));
        return;
      }
      ...
      if (!javaPath) {
        ...
        if (editor.selection && !editor.selection.isEmpty) {
          ...
          const inferredName = await this.inferMethodNameAtLine(editor.document, line);
          methodName = inferredName ?? undefined;
        }
      }
      
      if (!javaPath || !methodName) {
        vscode.window.showWarningMessage(vscode.l10n.t('generateXmlMethod.noMethodSelected'));
        return;
      }
      ...
      vscode.window.showErrorMessage(
        vscode.l10n.t('generateXmlMethod.failed', { error: error instanceof Error ? error.message : 'Unknown error' })
      );
    }
  }
   ...
   */
  private async generateMethod(javaPath: string, methodName: string): Promise<void> {
    // 1. 获取对应的 XML 文件路径
   ...
   * @returns 是否存在
   */
  private async methodExistsInXml(xmlPath: string, methodName: string): Promise<boolean> {
    try {
   ...
   * @param methodName - 方法名
   */
  private async createNewMapperXml(javaPath: string, methodName: string): Promise<void> {
    // 获取类名
    ...
    vscode.window.showInformationMessage(vscode.l10n.t('generateXmlMethod.created', { xmlPath }));
  }
   ...
   * @returns XML 内容
   */
  private generateMapperXmlTemplate(namespace: string, firstMethod: string): string {
