...
export class JavaExtensionAPI {
  ...
  public async initialize(context: vscode.ExtensionContext): Promise<void> {
    ...
    try {
      ...
      if (javaExt) {
        ...
        if (javaExt.exports) {
          this.logger.debug(
            "Java extension exports found, initializing API...",
          );
      ...
      this.logger.error(
        vscode.l10n.t("error.javaExtensionInitFailed", {
          error: String(error),
        }),
      );
      this.isActivated = false;
   ...
   */
  public async getMapperInterfaces(): Promise<
    Array<{ className: string; filePath: string }>
  > {
    if (!this.isReady) {
      this.logger.warn(
        "Java extension API not ready, cannot get Mapper interfaces",
      );
      return [];
    }
    ...
    try {
      ...
      for (const javaFile of javaFiles) {
        try {
          ...
          this.logger.error(
            `Error processing Java file ${javaFile.fsPath}:`,
            error as Error,
          );
        }
      }
      ...
      this.logger.error(
        vscode.l10n.t("error.mapperInterfacesFailed", { error: String(error) }),
      );
      return [];
    }
  }
  ...
  public async findJavaFileByClassName(
    ...
    if (!this.isReady) {
      this.logger.warn(
        "Java extension API not ready, using fallback method to find Java file",
      );
    ...
    try {
        ...
        try {
          ...
          if (classNameMatch && classNameMatch[1]) {
            ...
            if (fullClassName === className) {
              this.logger.debug(
                `Found Java file for class ${className}: ${javaFile.fsPath}`,
              );
      ...
      this.logger.debug(
        `No exact match found for ${className}, trying simple class name`,
      );
      const simpleClassName = className.substring(
      ...
      for (const javaFile of javaFiles) {
        try {
          ...
          if (classNameMatch && classNameMatch[1] === simpleClassName) {
            this.logger.debug(
              `Found Java file for simple class name ${simpleClassName}: ${javaFile.fsPath}`,
            );
            return javaFile.fsPath;
          }
        } catch (error) {
      ...
      this.logger.error(
        vscode.l10n.t("error.javaFileNotFound", {
          className,
          error: String(error),
        }),
      );
   ...
   * @returns Java文件路径，如果未找到则返回undefined
   */
  private async findJavaFileByClassNameFallback(
    className: string,
  ): Promise<string | undefined> {
    try {
      this.logger.debug(
        `Using fallback method to find Java file for class: ${className}`,
      );
      ...
      this.logger.error(
        `Error in fallback method for finding Java file:`,
        error as Error,
      );
      return undefined;
    }
   ...
   */
  public async getResourcePathForClass(
    className: string,
  ): Promise<string | undefined> {
    if (!this.isReady) {
      this.logger.warn(
        "Java extension API not ready, cannot get resource path",
      );
      return undefined;
    }
    ...
    try {
      ...
      if (javaFilePath) {
        ...
        this.logger.debug(
          `Resource path for class ${className}: ${resourcePath}`,
        );
        return resourcePath;
      }
      ...
      this.logger.error(
        vscode.l10n.t("error.resourcePathFailed", {
          className,
          error: String(error),
        }),
      );
   ...
   * @returns 是否成功导航
   */
  public async navigateToMethod(
    javaFilePath: string,
    methodName: string,
  ): Promise<boolean> {
    if (!this.isReady) {
      this.logger.warn(
        "Java extension API not ready, cannot navigate to method",
      );
      return false;
    }
    ...
    try {
      this.logger.debug(
        `Navigating to method ${methodName} in ${javaFilePath} using Java Extension API`,
      );
      ...
      while ((match = methodRegex.exec(content)) !== null) {
        ...
        this.logger.debug(
          `Successfully navigated to method ${methodName} in ${javaFilePath}`,
        );
        return true;
      }
      ...
      this.logger.error(
        vscode.l10n.t("error.navigateToMethodFailed", {
          methodName,
          error: String(error),
        }),
      );
