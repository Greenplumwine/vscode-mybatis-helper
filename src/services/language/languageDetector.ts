...
export class ExtensionDetectionStrategy implements ILanguageDetectionStrategy {
    detect(document: vscode.TextDocument, position?: vscode.Position): LanguageDetectionResult | null {
    ...
    private isMyBatisMapper(document: vscode.TextDocument): boolean {
        ...
        return /<!DOCTYPE\s+mapper\s+PUBLIC\s+["']-\/\/mybatis\.org\/\/DTD Mapper/.test(headerContent) ||
...
export class ContentDetectionStrategy implements ILanguageDetectionStrategy {
    detect(document: vscode.TextDocument, position?: vscode.Position): LanguageDetectionResult | null {
        ...
        // SQL 文件特征
        if (/^\s*(SELECT|INSERT|UPDATE|DELETE|CREATE|DROP|ALTER)\s+/i.test(content)) {
            return {
                language: LanguageType.SQL,
                confidence: 0.85
            };
...
export class MyBatisMapperDetectionStrategy implements ILanguageDetectionStrategy {
    private readonly MYBATIS_SIGNATURE = /<mapper\s+namespace\s*=\s*["'][^"']+["']/;
    private readonly MYBATIS_DTD = /<!DOCTYPE\s+mapper\s+PUBLIC\s+["']-\/\/mybatis\.org/;
...
export class LanguageDetector {
    ...
    public isJava(document: vscode.TextDocument): boolean {
        return this.detectLanguage(document) === LanguageType.JAVA;
    }
    ...
    public isXml(document: vscode.TextDocument): boolean {
        return this.detectLanguage(document) === LanguageType.XML;
    }
    ...
    public isSql(document: vscode.TextDocument): boolean {
        return this.detectLanguage(document) === LanguageType.SQL;
    }
    ...
    public isMyBatisMapper(document: vscode.TextDocument): boolean {
        ...
        return result?.metadata?.isMyBatisMapper === true;
    }
    ...
    public detectByPath(filePath: string): LanguageDetectionResult {
        ...
        if (lowerPath.endsWith('.java')) {
            return { language: LanguageType.JAVA, confidence: 0.9 };
        }
        if (lowerPath.endsWith('.xml')) {
            return { language: LanguageType.XML, confidence: 0.9 };
        }
        if (lowerPath.endsWith('.sql')) {
            return { language: LanguageType.SQL, confidence: 0.9 };
        }
        
        return { language: LanguageType.UNKNOWN, confidence: 0 };
    }
