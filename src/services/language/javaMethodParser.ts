...
import { JavaMapperInfo, JavaMethodInfo, JavaParameter } from "../types";
...
class SourceCodeParser {
  ...
  private extractInterfaceInfo(
    cleanContent: string,
  ): { name: string; isInterface: boolean } | null {
  ...
  private extractMethods(
    ...
    const methodPattern =
      /(?:public|private|protected)?\s*(?:static|final|abstract)?\s*([\w<>,\s\[\]]+?)\s+(\w+)\s*\((.*?)\)\s*(?:throws\s+[\w,\s]+)?\s*[;{]/gs;
    ...
    if (filePath.includes("SysJobMapper")) {
      logger.debug(
        `[DEBUG] SysJobMapper cleanContent snippet:`,
        cleanContent.substring(0, 2000),
      );
      logger.debug(
        `[DEBUG] insertJob found in cleanContent:`,
        cleanContent.includes("insertJob"),
      );
    }
    ...
    for (const match of cleanContent.matchAll(methodPattern)) {
      ...
      const originalParamsStr = this.extractOriginalParamsStr(
        originalContent,
        originalIndex,
        methodName,
      );
      ...
      if (methodName === "insertJob") {
        logger.debug(
          `[DEBUG] insertJob originalParamsStr: "${originalParamsStr}"`,
        );
      }
      ...
      const signature = this.extractMethodSignature(
        originalContent,
        originalIndex,
      );
    ...
    logger.debug(
      `Extracted ${methods.length} methods from ${filePath}: ${methods.map((m) => m.name).join(", ")}`,
    );
    return methods;
  }
  ...
  private mapCleanIndexToOriginal(
    ...
    if (!occurrences || occurrences.length === 0) {
      // 缓存未命中，回退到简单查找
      return this.fallbackIndexMapping(
        cleanContent,
        originalContent,
        cleanIndex,
        methodName,
    ...
    while (originalOccurrenceCount < cleanOccurrenceCount) {
      const idx = originalContent.indexOf(
        methodName + "(",
        originalSearchIndex,
      );
      if (idx === -1) {
   ...
   */
  private extractOriginalParamsStr(
    content: string,
    startIndex: number,
    methodName: string,
  ): string {
...
export class JavaMethodParser {
   ...
   * 支持简单类名和全限定名
   */
  private async findClassFileInProject(
    className: string,
  ): Promise<string | null> {
    try {
      // 获取所有工作区
      ...
      for (const folder of workspaceFolders) {
        ...
        for (const file of files) {
          ...
          // 验证文件内容是否包含类定义
          if (
            await this.verifyClassDefinition(
              filePath,
              className,
              simpleClassName,
            )
  ...
  private async verifyClassDefinition(
    ...
    try {
      ...
      const classPattern = new RegExp(
        `(class|interface|enum|record)\\s+${simpleName}\\b`,
      );
   ...
   * @returns 属性信息数组（包含名称和类型）
   */
  private async extractPropertiesFromClass(
    filePath: string,
  ): Promise<Array<{ name: string; type: string }>> {
    try {
      const content = await fs.readFile(filePath, "utf-8");
      ...
      logger.debug(
        `Extracted ${properties.length} properties from ${filePath}`,
      );
      return properties;
    } catch (error) {
  ...
  async parseMethod(filePath: string, methodName: string): Promise<any | null> {
    ...
    logger.debug(
      `parseMethod: Looking for ${methodName}, found ${info.methods?.length || 0} methods`,
    );
    if (info.methods?.length > 0) {
      logger.debug(
        `parseMethod: Available methods: ${info.methods.map((m) => m.name).join(", ")}`,
      );
    }
    ...
    return {
      ...
      isCollection:
        methodInfo.returnType.includes("List") ||
