...
function extractWhereConditions(methodName: string): WhereCondition[] {
  ...
  for (let i = 0; i < conditionPart.length; i++) {
    ...
    if (
      remaining.toLowerCase().startsWith("and") &&
      remaining.length > 3 &&
      remaining[3] === remaining[3].toUpperCase()
    ) {
      if (current) {
        parts.push(current);
        current = "";
    ...
    if (
      remaining.toLowerCase().startsWith("or") &&
      remaining.length > 2 &&
      remaining[2] === remaining[2].toUpperCase()
    ) {
      if (current) {
        parts.push(current);
        current = "";
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (lowerPart === "and") {
      ...
      lowerPart === "is" &&
      parts[i + 1]?.toLowerCase() === "not" &&
      parts[i + 2]?.toLowerCase() === "null"
    ) {
      // 处理 IsNotNull
      currentOperator = "IS NOT NULL";
      i += 2; // 跳过 'Not' 和 'Null'
...
export class TemplateEngine implements ITemplateEngine {
  ...
  registerTemplate(type: TemplateType, template: string): void {
    ...
    throw new Error(
      `Custom template registration not implemented. Type: ${type}`,
    );
  }
}
