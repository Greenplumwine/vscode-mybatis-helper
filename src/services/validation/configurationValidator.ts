...
export async function validateConfiguration(): Promise<ValidationResult> {
  ...
  logger.info(
    `[ConfigValidator] Validation completed: ${errors.length} errors, ${issues.length - errors.length} warnings`,
  );
  return result;
}
    ...
    if (workspaceFolders && workspaceFolders.length > 0) {
      ...
      if (!isWithinWorkspace) {
        issues.push({
          ...
          suggestion:
            "For security, consider placing XML files within the workspace",
        });
      }
    }
  }
    ...
    if (rule.xmlPattern) {
      try {
        ...
        issues.push({
          ...
          suggestion:
            "Use valid pattern with optional template variables like ${javaName}",
        });
      }
    }
  }
    ...
    for (let i = 0; i < pathPriority.priorityDirectories.length; i++) {
      ...
      if (dir.includes("..") || dir.includes("~")) {
        issues.push({
          ...
          suggestion:
            'Avoid using ".." or "~" in path patterns. Use relative paths like "/src/" or "/main/"',
        });
      }
    }
  }
    ...
    for (let i = 0; i < pathPriority.excludeDirectories.length; i++) {
      ...
      if (dir.includes("..") || dir.includes("~")) {
        issues.push({
          ...
          message: `Suspicious path pattern: "${dir}"`,
          suggestion:
            'Avoid using ".." or "~" in path patterns. Use relative paths like "/target/" or "/build/"',
        });
      }
    }
  }
