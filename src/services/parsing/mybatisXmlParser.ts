...
export class MyBatisXmlParser {
     ...
     * @returns MyBatis XML 信息
     */
    private parseXmlContent(content: string, filePath: string): MyBatisXmlInfo | null {
        try {
            // 提取命名空间
    ...
    private extractNamespace(content: string): string | null {
        const match = content.match(/<mapper[^>]*namespace\s*=\s*["']([^"']+)["']/i);
    ...
    private parseResultMaps(content: string): ResultMapInfo[] {
        ...
        const regex = /<resultMap[^>]*id\s*=\s*["']([^"']+)["'][^>]*type\s*=\s*["']([^"']+)["']/gi;
     ...
     */
    public findForeachContext(content: string, line: number): ForeachContext | null {
        // 查找所有 foreach 标签
        ...
        // 找到包含当前行的 foreach
        const containingForeach = matches.find(f => line >= f.startLine && line <= f.endLine);
