...
export class EnhancedJavaMethodParser {
    ...
    private fileCache = new Map<string, { mtime: number; properties: ObjectProperty[] }>();
    ...
    public parseMethodParameters(methodSignature: string): JavaParameter[] {
        // 提取参数部分：从第一个 ( 到最后一个 )
        const paramsMatch = methodSignature.match(/\((.*)\)\s*(?:throws\s+[\w,\s]+)?\s*[;{]$/s);
        if (!paramsMatch) {
            return [];
        }
        ...
        while (i < tokens.length) {
            ...
            if (hasParamAnnotation) {
                ...
                if (paramAnnotation) {
                    ...
                    const valueMatch = paramAnnotation.match(/@Param\s*\(\s*["']?([^"')\s]+)["']?\s*\)/);
                    if (valueMatch) {
     ...
     * 验证文件包名是否与类名匹配
     */
    private async verifyPackageName(filePath: string, className: string): Promise<boolean> {
        try {
    ...
    private extractPropertiesFromSource(content: string): ObjectProperty[] {
        ...
        const fieldPattern = /(?:private|protected|public)\s+(?:static\s+)?(?:final\s+)?(?:transient\s+)?([\w<>,\s\[\]?]+)\s+(\w+)\s*;/g;
        let match;
    ...
    public async findClassFile(className: string): Promise<string | undefined> {
        ...
        if (mavenRepoPath) return mavenRepoPath;
     ...
     * 在 Maven 本地仓库中查找类
     */
    private async findInMavenRepo(className: string): Promise<string | undefined> {
     ...
     * 在 JAR 包中查找类
     */
    private async findClassInJar(className: string, searchDir: string): Promise<string | undefined> {
     ...
     * @returns 属性信息数组
     */
    public async parseWithJavap(classFilePath: string): Promise<ObjectProperty[]> {
        // 验证路径安全性
        ...
        try {
            ...
            const { stdout } = await execFileAsync('javap', ['-p', '-public', classFilePath], {
                timeout: 3000,  // 3 秒超时
    ...
    private parseJavapOutput(output: string): ObjectProperty[] {
        ...
        for (const line of lines) {
            ...
            const getterMatch = trimmed.match(/public\s+([\w<>,\s\[\]]+)\s+get(\w+)\s*\(\s*\)/);
            if (getterMatch) {
                const returnType = getterMatch[1].trim();
                const propName = getterMatch[2].charAt(0).toLowerCase() + getterMatch[2].slice(1);
                properties.set(propName, returnType);
                continue;
            }

            // 解析 boolean getter：public boolean isActive();
            const booleanGetterMatch = trimmed.match(/public\s+boolean\s+is(\w+)\s*\(\s*\)/);
            if (booleanGetterMatch) {
                const propName = booleanGetterMatch[1].charAt(0).toLowerCase() + booleanGetterMatch[1].slice(1);
                properties.set(propName, 'boolean');
            }
        }
     ...
     * @returns 属性信息数组
     */
    public async getObjectProperties(className: string): Promise<ObjectProperty[]> {
        // 基本类型直接返回空数组
     ...
     * 实际执行属性获取
     */
    private async doGetObjectProperties(className: string): Promise<ObjectProperty[]> {
        ...
        try {
            ...
            if (sourceFile) {
                ...
                if (props.length > 0) {
                    ...
                    logger.debug(`Source parsed ${className}: ${props.length} properties`);
                    return props;
                }
            }
